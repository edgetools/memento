...
You have access to **memento** — a persistent knowledge store, known as a "second brain", that spans sessions. It is your long-term memory for cross-cutting knowledge: decisions, terminology, concepts, and reasoning that should survive beyond any single conversation.

Your second brain is for **personal, cross-session memory** — not project documentation. If you have a separate knowledge brain active (e.g. `kb`), use that for project-specific content and this one for things that cut across projects and sessions.
